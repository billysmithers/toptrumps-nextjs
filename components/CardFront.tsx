...
const capabilityIsSelectable = (isPlayersTurn, capabilityToUse, capability): boolean => {
    return isPlayersTurn && (! capabilityToUse || capabilityToUse === capability.capability)
}

export const CardFront: FC<CardProps> = ({card, isPlayersTurn, shouldDisplayValues, capabilityToUse}) =>
            ...
            <table className="text-xs my-3">
                ...
                {card.capabilities.map((capability, index) => (
                    <tr className={ index % 2 === 0 ? ' bg-yellow-300' : 'bg-yellow-100' }
                        ...
                        role={capabilityIsSelectable(isPlayersTurn, capabilityToUse, capability) ? 'button' : 'row'}
                    >
